@if($alert)
    <div id="sticky-banner" tabindex="-1" class="w-full flex justify-around p-2 border-b border-gray-200 bg-blue-100 dark:bg-gray-700 dark:border-gray-600">
        <div class="flex items-center mx-auto text-center">
            <p class="flex items-center text-sm text-center font-normal text-gray-500 dark:text-gray-400">
            <span class="inline-flex p-1 mr-3 bg-gray-200 rounded-full dark:bg-gray-600">
                ...
                <span class="sr-only">{{$alert->title}}</span>
            </span>
                <span><a href="{{route('alert.show',$alert->slug)}}" class="inline font-medium text-blue-600 underline dark:text-blue-500 underline-offset-2 decoration-600 dark:decoration-500 decoration-solid hover:no-underline">{{$alert->title}}</a></span>
            </p>
        </div>
