...
@section('content')
    <div class="container px-6 mx-auto grid">
        ...
        <a href="{{route('bases.create')}}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mr-auto">
        ...
        <div class="w-full overflow-hidden rounded-lg shadow-xs" >
                    ...
                    <table class="w-full ">
                        ...
                        @foreach($bases as $base)
                            <tr class="text-gray-700 dark:text-gray-400">
                                ...
                                <td class="px-4 py-3 text-sm">
                                    @if($base->photo)
                                        <a href="{{route('bases.show',$base->id)}}" title="{{$base->title}}"> <img src="{{$base->photo->address}}"  height="140" width="100" alt="" class="image-grayscale mx-auto"></a>
                                    @else
                                        <a href="{{route('bases.show',$base->id)}}" title="{{$base->title}}"><img src="{{url('/images/no-image.jpg')}}" height="100" width="100" alt="" class="rounded-full mx-auto"></a>
                                    @endif
                                </td>
                                ...
                                <td class="px-4 py-3 text-sm">
                                    @if(config('app.locale')=='fa')
                                        {{verta()->instance($base->created_at)->format('%d %B %Y')}}
                                    @else
                                        {{ date('d-M-y', strtotime($base->created_at))}}
                                    @endif
                                </td>
                                <td class="px-4 py-3">
                                    <div class=" text-xl flex justify-center">
                                    <a href="{{route('bases.show',$base->id)}}" class="text-blue-500 mx-auto" title="{{__('dashboard.show')}}">
                                        <i class="fa fa-eye"></i>
                                    </a>
                                    <a href="{{route('bases.edit',$base->id)}}" class="text-yellow-900 mx-auto" title="{{__('dashboard.edit')}}">
                                        <i class="fa fa-edit"></i>
                                    </a>

                                        <form action="{{route('bases.destroy',$base->id)}}" class="mx-auto" method="POST">
                                            @csrf
                                            {{method_field('DELETE')}}
            ...
            @if(!$bases->isEmpty())
                ...
                <span class="flex items-center col-span-3">
                  {{__('dashboard.number')}}  {{$bases->count()}}
                </span>
                  ...
                  <nav aria-label="Table navigation">
                    <ul class="inline-flex items-center px-4" dir="ltr">
                        {!! $bases->appends(Request::except('page'))->render() !!}
                    </ul>
                  </nav>
                </span>
                </div>
