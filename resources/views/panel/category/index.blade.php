...
@section('content')
    <div class="container px-6 mx-auto grid">
        ...
        <a href="{{route('categories.create')}}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mr-auto">
            <i class="fa fa-plus"></i>
            {{__('dashboard.create').' '.__('dashboard.category')}}
        </a>
        <!-- New Table -->
        <div class="w-full overflow-hidden rounded-lg shadow-xs" >
                    ...
                    <table class="w-full ">
                        ...
                        @foreach($categories as $category)
                            <tr class="text-gray-700 dark:text-gray-400">
                                ...
                                <td class="px-4 py-3 text-sm">
                                    @if($category->photo)
                                        <a href="{{route('categories.show',$category->id)}}" title="{{$category->name}}"> <img src="{{$category->photo->address}}"  height="140" width="100" alt="" class="image-grayscale mx-auto"></a>
                                    @else
                                        <a href="{{route('categories.show',$category->id)}}" title="{{$category->name}}"><img src="{{url('/images/no-image.jpg')}}" height="100" width="100" alt="" class="rounded-full mx-auto"></a>
                                    @endif
                                </td>
                                ...
                                <td class="px-4 py-3">
                                    <div class=" text-xl flex justify-center">
                                        <a href="{{route('categories.show',$category->id)}}" class="text-blue-500 mx-auto" title="{{__('dashboard.show')}}">
                                            <i class="fa fa-eye"></i>
                                        </a>
                                        <a href="{{route('categories.edit',$category->id)}}" class="text-yellow-900 mx-auto" title="{{__('dashboard.edit')}}">
                                            <i class="fa fa-edit"></i>
                                        </a>
                                        <form action="{{route('categories.destroy',$category->id)}}" class="mx-auto" method="POST">
                                            @csrf
                                            {{method_field('DELETE')}}
                            ...
                            @if(count($category->children) > 0)
                                @include('panel.partials.category_child',['categories'=>$category->children,'level'=>1])
                            @endif
                        @endforeach
                        </tbody>
                    </table>
            ...
            @if(!$categories->isEmpty())
                ...
                <span class="flex items-center col-span-3">
                  {{__('dashboard.number')}}  {{$categories->count()}}
                </span>
