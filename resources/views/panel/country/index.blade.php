...
@section('content')
    <div class="container px-6 mx-auto grid">
        ...
        <a href="{{route('countries.create')}}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mr-auto">
            <i class="fa fa-plus"></i>
            {{__('dashboard.create').' '.__('dashboard.country')}}
        </a>
        <!-- New Table -->
        <div class="w-full overflow-hidden rounded-lg shadow-xs" >
                ...
                @if(!$countries->isEmpty())
                    <table class="w-full ">
                        ...
                        <tr class="text-sm font-bold tracking-wide text-center text-gray-500 uppercase border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800" >
                            ...
                            <th class="px-4 py-3">@sortablelink('country_name',__('dashboard.country_name'))</th>
                            ...
                            <tr class="text-gray-700 dark:text-gray-400">
                                ...
                                <td class="px-4 py-3 text-sm">
                                    @if(config('app.locale')=='fa')
                                        {{verta()->instance($country->created_at)->format('%d %B %Y')}}
                                    @else
                                ...
                                <td class="px-4 py-3">
                                    ...
                                    <a href="{{route('countries.show',$country->id)}}" class="text-blue-500 mx-auto" title="{{__('dashboard.show')}}">
                                        <i class="fa fa-eye"></i>
                                    </a>
                                    <a href="{{route('countries.edit',$country->id)}}" class="text-yellow-900 mx-auto" title="{{__('dashboard.edit')}}">
                                        <i class="fa fa-edit"></i>
                                    </a>
                                    <form action="{{route('countries.destroy',$country->id)}}" class="mx-auto" method="POST">
                                            @csrf
                                            {{method_field('DELETE')}}
            ...
            @if(!$countries->isEmpty())
                ...
                <span class="flex items-center col-span-3">
                  {{__('dashboard.number')}}  {{$countries->count()}}
                </span>
