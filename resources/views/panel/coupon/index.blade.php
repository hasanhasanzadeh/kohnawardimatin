...
@section('content')
    <div class="container px-6 mx-auto grid">
        ...
        <a href="{{route('coupons.create')}}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mr-auto">
            <i class="fa fa-plus"></i>
            {{__('dashboard.create').' '.__('dashboard.coupon')}}
        </a>
        <!-- New Table -->
        <div class="w-full overflow-hidden rounded-lg shadow-xs" >
                ...
                @if(!$coupons->isEmpty())
                    <table class="w-full ">
                        ...
                        <tr class="text-xs font-bold tracking-wide text-center text-gray-500 uppercase border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800" >
                            ...
                            <th class="px-4 py-3">@sortablelink('discount','درصد تخفیف')</th>
                            <th class="px-4 py-3">@sortablelink('status', __('dashboard.status'))</th>
                            <th class="px-4 py-3">@sortablelink('expired_at', 'تاریخ انقضاء')</th>
                        ...
                        @foreach($coupons as $coupon)
                            <tr class="text-gray-700 dark:text-gray-400">
                                ...
                                <td class="px-4 py-3 text-sm">
                                    @if($coupon->photo)
                                        <a href="{{route('coupons.show',$coupon->id)}}" title="{{$coupon->title}}"> <img src="{{$coupon->photo->address}}"  height="140" width="100" alt="" class="image-grayscale mx-auto"></a>
                                    @else
                                        <a href="{{route('coupons.show',$coupon->id)}}" title="{{$coupon->title}}"><img src="{{url('/default-images/no-image.jpg')}}" height="100" width="100" alt="" class="rounded-full mx-auto"></a>
                                    @endif
                                </td>
                                ...
                                <td class="px-4 py-3">
                                    ...
                                    <a href="{{route('coupons.show',$coupon->id)}}" class="text-blue-500 mx-auto" title="{{__('dashboard.show')}}">
                                        <i class="fa fa-eye"></i>
                                    </a>
                                    <a href="{{route('coupons.edit',$coupon->id)}}" class="text-yellow-900 mx-auto" title="{{__('dashboard.edit')}}">
                                        <i class="fa fa-edit"></i>
                                    </a>
                                    <form action="{{route('coupons.destroy',$coupon->id)}}" class="mx-auto" method="POST">
                                            @csrf
                                            {{method_field('DELETE')}}
            ...
            @if(!$coupons->isEmpty())
                ...
                <span class="flex items-center col-span-3">
                  {{__('dashboard.number')}}  {{$coupons->count()}}
                </span>
