                    ...
                    <table class="w-full ">
                        ...
                        @foreach($customers as $customer)
                            <tr class="text-gray-700 dark:text-gray-400">
                                <td class="px-4 py-3 text-sm">{{++$row}}</td>
                                <td class="px-4 py-3 text-sm">
                                    @if($customer->photo)
                                        <img src="{{$customer->photo->address}}"  height="70" width="70" alt="" class="rounded-full w-12 h-12 mx-auto">
                                    @else
                                ...
                                <td class="px-4 py-3">
                                    ...
                                    <a href="{{ route('users.show', $customer->id) }}" class="text-indigo-400 mx-auto" title="{{__('dashboard.permissions')}}">
                                        ...
                                        <form  action="{{route('customers.destroy',$customer->id)}}" class="mx-auto" method="POST">
                                            @csrf
                                            {{method_field('DELETE')}}
