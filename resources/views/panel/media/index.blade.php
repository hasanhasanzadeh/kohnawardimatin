...
@section('content')
    <div class="container px-6 mx-auto grid">
        ...
        <a href="{{route('medias.create')}}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mr-auto">
        ...
        <div class="w-full overflow-hidden rounded-lg shadow-xs" >
                ...
                @if(!$medias->isEmpty())
                    <table class="w-full ">
                        ...
                        <tr class="text-xs font-bold tracking-wide text-center text-gray-500 uppercase border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800" >
                            ...
                            <th class="px-4 py-3">@sortablelink('link', __('dashboard.link'))</th>
                        ...
                        @foreach($medias as $media)
                            <tr class="text-gray-700 dark:text-gray-400">
                                ...
                                <td class="px-4 py-3 text-sm">
                                    @if($media->photo)
                                        <a href="{{route('medias.show',$media->id)}}" title="{{$media->title}}"> <img src="{{$media->photo->address}}"  height="140" width="100" alt="" class="image-grayscale mx-auto"></a>
                                    @else
                                        <a href="{{route('medias.show',$media->id)}}" title="{{$media->title}}"><img src="{{url('/images/no-image.jpg')}}" height="100" width="100" alt="" class="rounded-full mx-auto"></a>
                                    @endif
                                </td>
                                ...
                                <td class="px-4 py-3">
                                    ...
                                    <a href="{{route('medias.show',$media->id)}}" class="text-blue-500 mx-auto" title="{{__('dashboard.show')}}">
                                        <i class="fa fa-eye"></i>
                                    </a>
                                    <a href="{{route('medias.edit',$media->id)}}" class="text-yellow-900 mx-auto" title="{{__('dashboard.edit')}}">
                                        <i class="fa fa-edit"></i>
                                    </a>

                                        <form action="{{route('medias.destroy',$media->id)}}" class="mx-auto" method="POST">
                                            @csrf
                                            {{method_field('DELETE')}}
            ...
            @if(!$medias->isEmpty())
                ...
                <span class="flex items-center col-span-3">
                  {{__('dashboard.number')}}  {{$medias->count()}}
                </span>
