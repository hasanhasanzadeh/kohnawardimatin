...
@section('content')
    <div class="container px-6 mx-auto grid">
        ...
        <a href="{{route('posts.create')}}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mr-auto">
        ...
        <div class="w-full overflow-hidden rounded-lg shadow-xs" >
                    ...
                    <table class="w-full ">
                        ...
                        @foreach($posts as $post)
                            <tr class="text-gray-700 dark:text-gray-400">
                                ...
                                <td class="px-4 py-3 text-sm">
                                    @if($post->photo)
                                        <a href="{{route('posts.show',$post->id)}}" title="{{$post->title}}"> <img src="{{$post->photo->address}}"  height="140" width="100" alt="" class="image-grayscale mx-auto"></a>
                                    @else
                                        <a href="{{route('posts.show',$post->id)}}" title="{{$post->title}}"><img src="{{url('/default-images/no-image.jpg')}}" height="100" width="100" alt="" class="rounded-full mx-auto"></a>
                                    @endif
                                </td>
                                ...
                                <td class="px-4 py-3 text-xs">
                                     <span class="text-xs font-semibold inline-block py-1 px-2 uppercase rounded text-emerald-600 bg-emerald-200 uppercase last:mr-0 mr-1">
                                            {{number_format($post->price,0).' '.'تومان'}}
                                     </span>
                                </td>
                                <td class="px-4 py-3 text-sm">
                                    @if($post->status)
                                ...
                                <td class="px-4 py-3">
                                    ...
                                    <a href="{{route('posts.show',$post->id)}}" class="text-blue-500 mx-auto" title="{{__('dashboard.show')}}">
                                        <i class="fa fa-eye"></i>
                                    </a>
                                    <a href="{{route('posts.edit',$post->id)}}" class="text-yellow-900 mx-auto" title="{{__('dashboard.edit')}}">
                                        <i class="fa fa-edit"></i>
                                    </a>

                                        <form action="{{route('posts.destroy',$post->id)}}" class="mx-auto" method="POST">
                                            @csrf
                                            {{method_field('DELETE')}}
            ...
            @if(!$posts->isEmpty())
                ...
                <span class="flex items-center col-span-3">
                  {{__('dashboard.number')}}  {{$posts->count()}}
                </span>
