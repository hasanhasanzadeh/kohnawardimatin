...
@section('content')
    <div class="container px-6 mx-auto grid">
        ...
        <a href="{{route('roles.create')}}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mr-auto">
            <i class="fa fa-plus"></i>
            {{__('dashboard.create').' '.__('dashboard.role')}}
        </a>
        <!-- New Table -->
        <div class="w-full overflow-hidden rounded-lg shadow-xs" >
                        ...
                        @foreach($roles as $role)
                            <tr class="text-gray-700 dark:text-gray-400">
                                ...
                                <td class="px-4 py-3">
                                    ...
                                    <a href="{{route('roles.show',$role->id)}}" class="text-blue-500 mx-auto" title="{{__('dashboard.show')}}">
                                        <i class="fa fa-eye"></i>
                                    </a>
                                    <a href="{{route('roles.edit',$role->id)}}" class="text-yellow-900 mx-auto" title="{{__('dashboard.edit')}}">
                                        <i class="fa fa-edit"></i>
                                    </a>

                                        <form action="{{route('roles.destroy',$role->id)}}" class="mx-auto" method="POST">
                                            @csrf
                                            {{method_field('DELETE')}}
            ...
            @if(!$roles->isEmpty())
                ...
                <span class="flex items-center col-span-3">
                  {{__('dashboard.number')}}  {{$roles->count()}}
                </span>
