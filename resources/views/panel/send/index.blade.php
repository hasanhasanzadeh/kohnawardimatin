...
@section('content')
    <div class="container px-6 mx-auto grid">
        ...
        <a href="{{route('sends.create')}}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mr-auto">
        ...
        <div class="w-full overflow-hidden rounded-lg shadow-xs" >
                ...
                @if(!$sends->isEmpty())
                    <table class="w-full ">
                        ...
                        <tr class="text-sm font-bold tracking-wide text-center text-gray-500 uppercase border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800" >
                            ...
                            <th class="px-4 py-3">{{__('dashboard.full_name')}}</th>
                            <th class="px-4 py-3">{{__('dashboard.mobile')}}</th>
                            <th class="px-4 py-3">{{__('dashboard.mail')}}</th>
                            ...
                            <tr class="text-gray-700 dark:text-gray-400">
                                ...
                                <td class="px-4 py-3 text-sm">
                                    @if(config('app.locale')=='fa')
                                        {{verta()->instance($send->created_at)->format('%d %B %Y')}}
                                    @else
                                        {{ date('d-M-y', strtotime($send->created_at))}}
                                    @endif
                                </td>
                                <td class="px-4 py-3">
                                    <div class=" text-xl flex justify-center">
                                    <a href="{{route('sends.edit',$send->id)}}" class=" @if($send->status==1) text-blue-600 @else text-red-600 @endif mx-auto" title="{{__('dashboard.status')}}">
                                        @if($send->status==1)
                                        ...
                                        <form action="{{route('sends.destroy',$send->id)}}" class="mx-auto" method="POST">
                                            @csrf
                                            {{method_field('DELETE')}}
            ...
            @if(!$sends->isEmpty())
                ...
                <span class="flex items-center col-span-3">
                  {{__('dashboard.number')}}  {{$sends->count()}}
                </span>
                  ...
                  <nav aria-label="Table navigation ">
                    <ul class="inline-flex items-center px-4 text-center" dir="ltr">
                        {!! $sends->appends(Request::except('page'))->render() !!}
                    </ul>
                  </nav>
                </span>
                </div>
