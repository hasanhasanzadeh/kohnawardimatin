                ...
                @if(!$sliders->isEmpty())
                    <table class="w-full ">
                        ...
                        <tr class="text-sm font-bold tracking-wide text-center text-gray-500 uppercase border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800" >
                            ...
                            <th class="px-4 py-3">{{__('dashboard.category')}}</th>
                        ...
                        @foreach($sliders as $slider)
                            <tr class="text-gray-700 dark:text-gray-400">
                                ...
                                <td class="px-4 py-3 text-sm">
                                    @if($slider->photo)
                                        <a href="{{route('sliders.show',$slider->id)}}" title="{{$slider->name}}"> <img src="{{$slider->photo->address}}"  height="140" width="100" alt="" class="image-grayscale mx-auto"></a>
                                    @else
                                        <a href="{{route('sliders.show',$slider->id)}}" title="{{$slider->name}}"><img src="{{url('/default-images/no-image.jpg')}}" height="100" width="100" alt="" class="rounded-full mx-auto"></a>
                                    @endif
                                </td>
                                ...
                                <td class="px-4 py-3 text-xs">
                                    @switch($slider->rang)
                                        ...
                                        @case('link') @lang('dashboard.link1') @break
                                    @endswitch
                                </td>
                                <td class="px-4 py-3 text-sm">
                                    @if($slider->status)
